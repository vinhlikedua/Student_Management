            ...
            <div class="col-sm-12 col-md-6">
                <form method="get" action="index.php">
                    ...
                    <input type="text" name="product_search" id="product_search"
                        value="<?= htmlentities($keyword ?? ''); ?>" placeholder="" />
                    <button type="submit" class="btn btn-primary btn-sm">Search</button>
                    <a class="btn btn-info btn-sm" href="index.php?c=courses">Refesh</a>
                </form>
            </div>
        </div>
        ...
        <table class="table table-bordered table-striped my-3">
            ...
            <tbody>
                ...
                <tr>
                    <td><?= $item['id']; ?></td>
                    <td><?= $item['name']; ?></td>
                    <td><?= isset($departmentNames[$item['department_id']]) ? $departmentNames[$item['department_id']] : ''; ?>
                    </td>
                    <td><?= $item['status'] == 1 ? 'Active' : 'Deactive'; ?></td>
                    <td>
                        <a class="btn btn-primary btn-sm"
                            href="index.php?c=courses&m=edit&id=<?= isset($item['id']) ? $item['id'] : ''; ?>">Edit</a>
                    </td>
                    <td>
                        <a class="btn btn-danger btn-sm"
                            href="index.php?c=courses&m=delete&id=<?= isset($item['id']) ? $item['id'] : ''; ?>">Delete</a>
                    </td>
                </tr>
                <?php endforeach; ?>
            </tbody>
